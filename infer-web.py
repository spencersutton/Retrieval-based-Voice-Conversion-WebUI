...
names = []
if weight_root:
    for p in Path(weight_root).iterdir():
        if p.suffix == ".pth":
            names.append(p.name)
index_paths = []
...
def lookup_indices(index_root):
    ...
    for p in root_path.rglob("*.index"):
...
uvr5_names = []
if weight_uvr5_root:
    for p in Path(weight_uvr5_root).iterdir():
        if p.suffix == ".pth" or "onnx" in p.name:
            uvr5_names.append(p.name.replace(".pth", ""))
...
def change_choices():
    ...
    index_paths = []
    if index_root:
        for p in Path(index_root).rglob("*.index"):
            if "trained" not in p.name:
                index_paths.append(str(p))
...
def preprocess_dataset(trainset_dir, exp_dir, sr, n_p):
    ...
    while 1:
        ...
        if done[0]:
            break
    with (log_dir / "preprocess.log").open("r") as f:
        log = f.read()
    logger.info(log)
    yield log
...
def extract_f0_feature(gpus, n_p, f0method, if_f0, exp_dir, version19, gpus_rmvpe):
    ...
    log_dir = now_dir / "logs" / exp_dir
    log_dir.mkdir(parents=True, exist_ok=True)
    (log_dir / "extract_f0_feature.log").write_text("")
    if if_f0:
...
def get_pretrained_models(path_str, f0_str, sr2):
    gen = Path(f"assets/pretrained{path_str}/{f0_str}G{sr2}.pth")
    dis = Path(f"assets/pretrained{path_str}/{f0_str}D{sr2}.pth")
    if_pretrained_generator_exist = gen.exists()
    if_pretrained_discriminator_exist = dis.exists()
    ...
    return (
        str(gen) if if_pretrained_generator_exist else "",
        str(dis) if if_pretrained_discriminator_exist else "",
    )
...
def change_version19(sr2, if_f0_3, version19):
    ...
    f0_str = "f0" if if_f0_3 else ""
    gen, dis = get_pretrained_models(path_str, f0_str, sr2)
...
def click_train(
    ...
    logger.info("Execute: " + cmd)
    p = Popen(cmd, shell=True, cwd=str(now_dir))
    p.wait()
...
def train_index(exp_dir1, version19):
    ...
    if not feature_dir.exists():
        return "请先进行特征提取!"
    listdir_res = sorted([p.name for p in feature_dir.iterdir() if p.is_file()])
    if len(listdir_res) == 0:
        return "请先进行特征提取！"
    infos = []
    npys = []
    for name in listdir_res:
        phone = np.load(str(feature_dir / name))
        npys.append(phone)
    ...
    faiss.write_index(
        ...
        str(
            exp_dir
            / f"trained_IVF{n_ivf}_Flat_nprobe_{index_ivf.nprobe}_{exp_dir1}_{version19}.index"
        ),
    )
    infos.append("adding")
    yield "\n".join(infos)
    ...
    faiss.write_index(
        ...
        str(
            exp_dir
            / f"added_IVF{n_ivf}_Flat_nprobe_{index_ivf.nprobe}_{exp_dir1}_{version19}.index"
        ),
    )
    infos.append(
    ...
    try:
        ...
        dst = (
            Path(outside_index_root)
            / f"{exp_dir1}_IVF{n_ivf}_Flat_nprobe_{index_ivf.nprobe}_{exp_dir1}_{version19}.index"
        )
